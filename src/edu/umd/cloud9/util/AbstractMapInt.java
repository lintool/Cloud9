...
import edu.umd.cloud9.util.MapInt.Entry;
...
public abstract class AbstractMapInt<K> implements MapInt<K> {
    /**
     * Sole constructor.  (For invocation by subclass constructors, typically
     * implicit.)
     */
    protected AbstractMapInt() {
    }
	...
	Iterator<MapInt.Entry<K>> i = entrySet().iterator();
	if (key==null) {
	    while (i.hasNext()) {
		Entry<K> e = i.next();
		if (e.getKey()==null)
    ...
    public void putAll(MapInt<? extends K> m) {
        for (MapInt.Entry<? extends K> e : m.entrySet())
	...
	if (keySet == null) {
		...
		public int size() {
		    return AbstractMapInt.this.size();
		}

		public boolean contains(Object k) {
		    return AbstractMapInt.this.containsKey(k);
		}
	    };
	}
	return keySet;
	...
	if (values == null) {
		...
		public boolean contains(int v) {
		    return AbstractMapInt.this.containsValue(v);
		}
	    };
	}
	return values;
	...
	if (!(o instanceof MapInt))
	    return false;
	MapInt<K> m = (MapInt<K>) o;
	if (m.size() != size())
	    return false;
    ...
    protected Object clone() throws CloneNotSupportedException {
        AbstractMapInt<K> result = (AbstractMapInt<K>)super.clone();
        result.keySet = null;
        result.values = null;
        return result;
    }
