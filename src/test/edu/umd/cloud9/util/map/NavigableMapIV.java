...
/**
 * A {@link SortedMap} extended with navigation methods returning the closest matches for given
 * search targets. Methods {@code lowerEntry}, {@code floorEntry}, {@code ceilingEntry}, and {@code
 * higherEntry} return {@code Map.Entry} objects associated with keys respectively less than, less
 * than or equal, greater than or equal, and greater than a given key, returning {@code null} if
 * there is no such key. Similarly, methods {@code lowerKey}, {@code floorKey}, {@code ceilingKey},
 * and {@code higherKey} return only the associated keys. All of these methods are designed for
 ...
 * The {@code descendingMap} method returns a view of the map with the senses of all relational and
 * directional methods inverted. The performance of ascending operations and views is likely to be
 * faster than that of descending ones. Methods {@code subMap}, {@code headMap}, and {@code tailMap}
 * differ from the like-named {@code SortedMap} methods in accepting additional arguments describing
 * whether lower and upper bounds are inclusive versus exclusive. Submaps of any {@code
 * NavigableMap} must implement the {@code NavigableMap} interface.
 * 
 * <p>
 * This interface additionally defines methods {@code firstEntry}, {@code pollFirstEntry}, {@code
 * lastEntry}, and {@code pollLastEntry} that return and/or remove the least and greatest mappings,
 * if any exist, else returning {@code null}.
 * 
 * <p>
 * Implementations of entry-returning methods are expected to return {@code Map.Entry} pairs
 * representing snapshots of mappings at the time they were produced, and thus generally do
 * <em>not</em> support the optional {@code Entry.setValue} method. Note however that it is possible
 * to change mappings in the associated map using method {@code put}.
 * 
 * <p>
 * Methods {@link #subMap(Object, Object) subMap(K, K)}, {@link #headMap(Object) headMap(K)}, and
 * {@link #tailMap(Object) tailMap(K)} are specified to return {@code SortedMap} to allow existing
 * implementations of {@code SortedMap} to be compatibly retrofitted to implement {@code
 * NavigableMap}, but extensions and implementations of this interface are encouraged to override
 * these methods to return {@code NavigableMap}. Similarly, {@link #keySet()} can be overriden to
 * return {@code NavigableSet}.
 * 
 * <p>
...
public interface NavigableMapIV<V> extends SortedMapIV<V> {
   ...
   */
  MapIV.Entry<V> lowerEntry(int key);

  /**
   * Returns the greatest key strictly less than the given key, or {@code null} if there is no such
   * key.
   * 
   * @param key
   *          the key
   ...
   *           if the specified key is null and this map does not permit null keys
   */
  MapIV.Entry<V> floorEntry(int key);
   ...
   *           if the specified key is null and this map does not permit null keys
   */
  MapIV.Entry<V> ceilingEntry(int key);
   ...
   *           if the specified key is null and this map does not permit null keys
   */
  MapIV.Entry<V> higherEntry(int key);

  /**
   * Returns the least key strictly greater than the given key, or {@code null} if there is no such
   * key.
   * 
   * @param key
   *          the key
  ...
  /**
   * Removes and returns a key-value mapping associated with the least key in this map, or {@code
   ...
   * backed by this map, so changes to the map are reflected in the descending map, and vice-versa.
   * If either map is modified while an iteration over a collection view of either map is in
   ...
   * The expression {@code m.descendingMap().descendingMap()} returns a view of {@code m}
   ...
   * in progress (except through the iterator's own {@code remove} operation), the results of the
   ...
   * {@code retainAll}, and {@code clear} operations. It does not support the {@code add} or {@code
   * addAll} operations.
   * 
   ...
   * iterator returns the keys in descending order. The set is backed by the map, so changes to the
   * map are reflected in the set, and vice-versa. If the map is modified while an iteration over
   ...
   * {@code fromExclusive} and {@code toExclusive} are both true. The returned map is backed by this
   * map, so changes in the returned map are reflected in this map, and vice-versa. The returned map
   * supports all optional map operations that this map supports.
   * 
   * <p>
   * The returned map will throw an {@code IllegalArgumentException} on an attempt to insert a key
   * outside of its range, or to construct a submap either of whose endpoints lie outside its range.
   * 
   * @param fromKey
   ...
   *         toKey}
   * @throws ClassCastException
   *           if {@code fromKey} and {@code toKey} cannot be compared to one another using this
   ...
   *           if {@code fromKey} is greater than {@code toKey}; or if this map itself has a
   *           restricted range, and {@code fromKey} or {@code toKey} lies outside the bounds of the
   *           range
   */
  NavigableMapIV<V> subMap(int fromKey, boolean fromInclusive, int toKey, boolean toInclusive);

  /**
   * Returns a view of the portion of this map whose keys are less than (or equal to, if {@code
   * inclusive} is true) {@code toKey}. The returned map is backed by this map, so changes in the
   * returned map are reflected in this map, and vice-versa. The returned map supports all optional
   ...
   * @throws ClassCastException
   *           if {@code toKey} is not compatible with this map's comparator (or, if the map has no
   *           comparator, if {@code toKey} does not implement {@link Comparable}). Implementations
   *           may, but are not required to, throw this exception if {@code toKey} cannot be
   *           compared to keys currently in the map.
   * @throws NullPointerException
   *           if {@code toKey} is null and this map does not permit null keys
   * @throws IllegalArgumentException
   *           if this map itself has a restricted range, and {@code toKey} lies outside the bounds
   *           of the range
   */
  NavigableMapIV<V> headMap(int toKey, boolean inclusive);

  /**
   * Returns a view of the portion of this map whose keys are greater than (or equal to, if {@code
   * inclusive} is true) {@code fromKey}. The returned map is backed by this map, so changes in the
   * returned map are reflected in this map, and vice-versa. The returned map supports all optional
   * map operations that this map supports.
   * 
   * <p>
   ...
   * @throws ClassCastException
   *           if {@code fromKey} is not compatible with this map's comparator (or, if the map has
   ...
   * @throws IllegalArgumentException
   *           if this map itself has a restricted range, and {@code fromKey} lies outside the
   *           bounds of the range
   */
  NavigableMapIV<V> tailMap(int fromKey, boolean inclusive);
   ...
   *           {@inheritDoc}
   */
  SortedMapIV<V> subMap(int fromKey, int toKey);
   ...
   *           {@inheritDoc}
   */
  SortedMapIV<V> headMap(int toKey);
   ...
   *           {@inheritDoc}
   */
  SortedMapIV<V> tailMap(int fromKey);
}
