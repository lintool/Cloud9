 ...
 * <tt>values</tt> methods). Several additional operations are provided to take advantage of the
 * ordering. (This interface is the map analogue of {@link SortedSet}.)
 * 
 * <p>
 * All keys inserted into a sorted map must implement the <tt>Comparable</tt> interface (or be
 * accepted by the specified comparator). Furthermore, all such keys must be <i>mutually
 * comparable</i>: <tt>k1.compareTo(k2)</tt> (or <tt>comparator.compare(k1, k2)</tt>) must not throw
 * a <tt>ClassCastException</tt> for any keys <tt>k1</tt> and <tt>k2</tt> in the sorted map.
 * Attempts to violate this restriction will cause the offending method or constructor invocation to
 ...
 * provided) must be <i>consistent with equals</i> if the sorted map is to correctly implement the
 * <tt>Map</tt> interface. (See the <tt>Comparable</tt> interface or <tt>Comparator</tt> interface
 * for a precise definition of <i>consistent with equals</i>.) This is so because the <tt>Map</tt>
 * interface is defined in terms of the <tt>equals</tt> operation, but a sorted map performs all key
 * comparisons using its <tt>compareTo</tt> (or <tt>compare</tt>) method, so two keys that are
 * deemed equal by this method are, from the standpoint of the sorted map, equal. The behavior of a
 * tree map <i>is</i> well-defined even if its ordering is inconsistent with equals; it just fails
 * to obey the general contract of the <tt>Map</tt> interface.
 * 
 * <p>
 * All general-purpose sorted map implementation classes should provide four "standard"
 * constructors: 1) A void (no arguments) constructor, which creates an empty sorted map sorted
 * according to the natural ordering of its keys. 2) A constructor with a single argument of type
 * <tt>Comparator</tt>, which creates an empty sorted map sorted according to the specified
 * comparator. 3) A constructor with a single argument of type <tt>Map</tt>, which creates a new map
 * with the same key-value mappings as its argument, sorted according to the keys' natural ordering.
 * 4) A constructor with a single argument of type <tt>SortedMap</tt>, which creates a new sorted
 * map with the same key-value mappings and the same ordering as the input sorted map. There is no
 * way to enforce this recommendation, as interfaces cannot contain constructors.
 * 
 * <p>
 * Note: several methods return submaps with restricted key ranges. Such ranges are
 * <i>half-open</i>, that is, they include their low endpoint but not their high endpoint (where
 * applicable). If you need a <i>closed range</i> (which includes both endpoints), and the key type
 * allows for calculation of the successor of a given key, merely request the subrange from
 * <tt>lowEndpoint</tt> to <tt>successor(highEndpoint)</tt>. For example, suppose that <tt>m</tt> is
 * a map whose keys are strings. The following idiom obtains a view containing all of the key-value
 ...
 * endpoint). The following idiom obtains a view containing all of the key-value mappings in
...
public interface SortedMapIV<V> extends MapIV<V> {
  /**
   * Returns a view of the portion of this map whose keys range from <tt>fromKey</tt>, inclusive, to
   * <tt>toKey</tt>, exclusive. (If <tt>fromKey</tt> and <tt>toKey</tt> are equal, the returned map
   ...
   *           if <tt>fromKey</tt> and <tt>toKey</tt> cannot be compared to one another using this
   ...
   *           <tt>fromKey</tt> or <tt>toKey</tt> cannot be compared to keys currently in the map.
   ...
   *           comparator, if <tt>toKey</tt> does not implement {@link Comparable}). Implementations
   *           may, but are not required to, throw this exception if <tt>toKey</tt> cannot be
   *           compared to keys currently in the map.
   * @throws NullPointerException
   *           if <tt>toKey</tt> is null and this map does not permit null keys
   * @throws IllegalArgumentException
   *           if this map itself has a restricted range, and <tt>toKey</tt> lies outside the bounds
   *           of the range
   */
   ...
   *           Implementations may, but are not required to, throw this exception if
   *           <tt>fromKey</tt> cannot be compared to keys currently in the map.
   * @throws NullPointerException
   *           if <tt>fromKey</tt> is null and this map does not permit null keys
   * @throws IllegalArgumentException
   *           if this map itself has a restricted range, and <tt>fromKey</tt> lies outside the
   *           bounds of the range
   */
