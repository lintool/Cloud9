...
public class Long2ObjectOpenHashMap <V> extends AbstractLong2ObjectMap <V> implements java.io.Serializable, Cloneable, Hash {
 ...
 public Long2ObjectOpenHashMap( final int n, final float f ) {
 ...
 /** Creates a new hash map with {@link Hash#DEFAULT_LOAD_FACTOR} as load factor.
	 ...
	 * @param n the expected number of elements in the hash map.
	 */
 public Long2ObjectOpenHashMap( final int n ) {
 ...
 private class MapIterator {
  ...
  int c = count;
  {
   final byte state[] = Long2ObjectOpenHashMap.this.state;
   final int n = state.length;
   if ( c != 0 ) while( pos < n && state[ pos ] != OCCUPIED ) pos++;
  }
  public boolean hasNext() {
   return c != 0 && pos < Long2ObjectOpenHashMap.this.state.length;
  }
 ...
 private final class MapEntrySet extends AbstractObjectSet<Long2ObjectMap.Entry <V> > implements FastEntrySet <V> {
  ...
  public boolean remove( final Object o ) {
   ...
   final Map.Entry<Long, V> e = (Map.Entry<Long, V>)o;
   final int i = findKey( ((e.getKey()).longValue()) );
   if ( i >= 0 ) Long2ObjectOpenHashMap.this.remove( e.getKey() );
   return i >= 0;
  }
  public int size() {
   return count;
