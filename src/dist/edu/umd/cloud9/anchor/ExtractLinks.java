...
import edu.umd.cloud9.anchor.normalize.AnchorTextNormalizer;
...
public class ExtractLinks extends PowerTool {
	...
	private static final Logger sLogger = Logger.getLogger(ExtractLinks.class);
	...
	public static class Map extends MapReduceBase implements
		...
		public static enum LinkCounter {
			...
			TEXT_TOO_LONG,	//number of lines of anchor text that are abnormally long
			PARSER_FAILED	//number of times the HTML parser fails
		};
		...
		private static final ArrayListWritable<AnchorText> arrayList = 
						new ArrayListWritable<AnchorText>();	//output value for the mappers
				
		private ClueWarcDocnoMapping mDocnoMapping = new ClueWarcDocnoMapping();	//docno mapping file
		...
		private AnchorTextNormalizer normalizer;
		...
		public void configure(JobConf job) {
			...
			includeInLinks = job.getBoolean("Ivory.IncludeInLinks", false);
		...
		public void map(IntWritable key, ClueWarcRecord doc,
				OutputCollector<Text, ArrayListWritable<AnchorText>> output, Reporter reporter) throws IOException {
			
			reporter.incrCounter(LinkCounter.INPUT_DOCS, 1);
			
			try {
				docno = mDocnoMapping.getDocno(doc.getHeaderMetadataItem("WARC-TREC-ID"));
	        ...
	        try {
				parser = new Parser();				//creating a new HTML parser
				parser.setInputHTML(doc.getContent());	//initializing the parser with new HTML content
		    ...
		    for (int i = 0; i < list.size (); i++) {
	    		...
	    		if(baseHost.equals(host)) {
    				...
    				arrayList.add(new AnchorText(AnchorTextConstants.INTERNAL_IN_LINK, anchor, docno));
	    			
	    		} else
    				arrayList.add(new AnchorText(AnchorTextConstants.EXTERNAL_IN_LINK, anchor, docno));
	    		...
	    		try {
	    			...
	    			keyWord.set(url);
	    			byte flag = arrayList.get(0).getFlag();
	    			arrayList.clear();
	        ...
	        arrayList.add(new AnchorText(AnchorTextConstants.DOCNO_FIELD, AnchorTextConstants.EMPTY_STRING, docno));
	        keyWord.set(base);
			output.collect(keyWord, arrayList);
			
			//keeping track of the number of documents that have actually been processed
	...
	public static final String[] RequiredParameters = { 
		...
		"Ivory.Mappers", 
		"Ivory.Reducers", 
		"Ivory.DocnoMappingFile",
		"Ivory.IncludeInLinks",
	...
	public int runTool() throws Exception {
		...
		sLogger.info("ExtractLinks");
		sLogger.info(" - input path: " + inputPath);
		sLogger.info(" - output path: " + outputPath);
		sLogger.info(" - mapping file: " + mappingFile);
		sLogger.info(" - include inlinks? " + conf.getBoolean("Ivory.IncludeInLinks", false));
