...
/**
 * <p>
 * Main driver program for extracting the Wt10g web graph, reverse web graph,
...
public class Wt10gDriver extends Configured implements Tool
{
	...
	public int run(String[] args) throws Exception
	{
		...
		for (int i = fromSegment; i <= toSegment; i++)
		{
			String inputPath = inputBase
					+ String.format(Locale.US, "WTX%03d", i);
			...
			int r = new Wt10gExtractLinks(conf).run();
		...
		for (int i = fromSegment; i < toSegment; i++)
		{
			inputPath += outputBase + outputExtractLinks+ String.format(Locale.US, "/WTX%03d/,", i);
		}
		
		inputPath += outputBase + outputExtractLinks+ String.format(Locale.US, "/WTX%03d/", toSegment);
	...
	{
		int res = ToolRunner.run(new Configuration(), new Wt10gDriver(), args);
		System.exit(res);
	}
}
