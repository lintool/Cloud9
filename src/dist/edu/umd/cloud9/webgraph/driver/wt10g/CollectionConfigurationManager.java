...
public class CollectionConfigurationManager
{
    ...
    private Class userSpecifiedDocnoMappingClass;
    ...
    public boolean setUserSpecifiedDocnoMapping(String className)
    {
	// It must be a class file
	if (!className.endsWith(".class"))
	    return false;
	...
	if (!GenericDocnoMappingInterface.class.isAssignableFrom(userClass))
	    return false;

	userSpecifiedDocnoMapping = true;
	userSpecifiedDocnoMappingClass = userClass;

	return true;
    }

    public DocnoMapping generateDocmapping(JobConf conf) throws Exception
    {
    ...
    public void applyJobConfig(JobConf conf) throws Exception
    {
	if (userSpecifiedInputFormat)
	{
	    conf.setInputFormat(userSpecifiedInputFormatClass);
	}
	else
	{
	    switch (tgtConf)
