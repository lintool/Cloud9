 ...
 * @section Description
 * 
 *          The univariate Gaussian distribution is an exponential family and, as a consequence, the
 *          probability density function is given by \f[ f(x; \mathbf{\Theta}) = \exp \left( \langle
 *          t(x), \mathbf{\Theta} \rangle - F(\mathbf{\Theta}) + k(x) \right) \f] where \f$
 *          \mathbf{\Theta} \f$ are the natural parameters. This class implements the different
 *          functions allowing to express a univariate Gaussian distribution as a member of an
 *          exponential family.
 * 
 * @section Parameters
 * 
 *          The parameters of a given distribution are: - Source parameters \f$\mathbf{\Lambda} = (
 *          \mu , \sigma^2 ) \in R \times R^+\f$ - Natural parameters \f$\mathbf{\Theta} = (
...
public final class UnivariateGaussian {
   ...
   * @return \f$ F(\mathbf{\Theta}) = -\frac{\theta_1^2}{4\theta_2} + \frac{1}{2} \log \left(
   *         -\frac{\pi}{\theta_2} \right) \f$
   */
   ...
   * @return \f$ \nabla F(\mathbf{\Theta}) = \left( -\frac{\theta_1}{2 \theta_2} , -\frac{1}{2
   *         \theta_2} + \frac{\theta_1^2}{4 \theta_2^2} \right) \f$
   */
  public PVector gradF(PVector T) {
    PVector gradient = new PVector(2);
    gradient.array[0] = -0.5d * T.array[0] / T.array[1];
    gradient.array[1] = 0.25d * (T.array[0] * T.array[0]) / (T.array[1] * T.array[1]) - 0.5d
        / T.array[1];
   ...
   * 
   * @param H expectation parameters \f$ \mathbf{H} = ( \eta_1 , \eta_2) \f$
   * @return \f$ \nabla G(\mathbf{H}) = \left( -\frac{\eta_1}{\eta_1^2-\eta_2} , \frac{1}{2
   *         (\eta_1^2-\eta_2)} \right) \f$
   */
   ...
   * @return natural parameters \f$ \mathbf{\Theta} = \left( \frac{\mu}{\sigma^2} ,
   *         -\frac{1}{2\sigma^2} \right) \f$
   */
   ...
   * 
   * @param T natural parameters \f$ \mathbf{\Theta} = ( \theta_1 , \theta_2 )\f$
   * @return source parameters \f$ \mathbf{\Lambda} = \left( -\frac{\theta_1}{2 \theta_2} ,
   *         -\frac{1}{2 \theta_2} \right) \f$
   */
