 ...
 *
 */
public class SortByHubRank extends Configured implements Tool {
	...
	private static final Logger sLogger = Logger.getLogger(SortByHubRank.class);
	...
	public int run(String[] args) throws Exception {
		...
		sLogger.info("Tool: HubSorter");
		sLogger.info(" - input path: " + inputPath);
		sLogger.info(" - output path: " + outputPath);
		sLogger.info(" - number of mappers: " + mapTasks);
		sLogger.info(" - number of reducers: " + reduceTasks);

		JobConf conf = new JobConf(SortByHubRank.class);
		conf.setJobName("HubSort");
		
		conf.setNumMapTasks(mapTasks);
		conf.setNumReduceTasks(reduceTasks);

		FileInputFormat.setInputPaths(conf, new Path(inputPath));
		// FileInputFormat.addInputPath(conf, new Path("/tmp/ccc_mmcgrath/tweets2"));
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new SortByHubRank(), args);
		System.exit(res);
	}
