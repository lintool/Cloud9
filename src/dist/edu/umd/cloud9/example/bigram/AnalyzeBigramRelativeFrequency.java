...
public class AnalyzeBigramRelativeFrequency {
	public static void main(String[] args) {
		...
		List<PairOfWritables<PairOfStrings, FloatWritable>> pairs = SequenceFileUtils.readDirectory(new Path(args[0]));
		...
		for (PairOfWritables<PairOfStrings, FloatWritable> p : pairs) {
			...
			if (bigram.getLeftElement().equals("light")) { list1.add(p);}
			if (bigram.getLeftElement().equals("contain")) { list2.add(p); }
		}
